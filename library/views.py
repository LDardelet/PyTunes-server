...
from .serializers import MusicSerializer, PlaylistSerializer, YtRefSerializer, ProfileSerializer
...
class PlaylistsListView(APIView):
    def get(self, request):
        return JsonResponse(PlaylistSerializer(Playlist.objects.all(), many=True).data, safe=False)
...
class PlaylistView(APIView):
    def get(self, request, playlist_id):
        ...
        if playlist is None:
            raise Http404(f"Playlist {playlist_id} does not exist")
        return JsonResponse(PlaylistSerializer(playlist).data, safe=False)

    def put(self, requets, playlist_id):
        playlist = get_playlist_from_id(playlist_id)
        if playlist is None:
            raise Http404(f"Playlist {playlist_id} does not exist")
        data = JSONParser().parse(request)
        ...
        for music_id, in_playlist in songs_updates:
            music = get_music_from_id(music_id)
            if music is None:
                raise HttpResponse(f"Music {music_id} does not exist", status = 204)
            if in_playlist:
                if not playlist.musics.contains(music):
                    playlist.musics.add(music)
                else:
                    raise HttpResponse(f"Music {music_id} already in this playlist", status = 204)
            else:
                if playlist.musics.contains(music):
                    playlist.musics.remove(music)
                else:
                    raise HttpResponse(f"Music {music_id} not in this playlist", status = 204)
            Update = True
